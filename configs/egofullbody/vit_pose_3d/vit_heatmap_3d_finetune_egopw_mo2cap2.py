...
evaluation = dict(
    res_folder='/CT/EgoMocap/work/EgocentricFullBody/work_dirs/vit_heatmap_3d_finetune_egopw_mo2cap2',
    metric='pa-mpjpe',
    save_best='pa-mpjpe',
    rule='less'
)
...
load_from = '/CT/EgoMocap/work/EgocentricFullBody/work_dirs/vit_256x256_heatmap_3d_egopw_finetune_mo2cap2_test/epoch_16.pth'

log_config = dict(
    interval=50,
    hooks=[
        dict(type='TextLoggerHook'),
        dict(type='WandbLoggerHook', init_kwargs=dict(project='vit_heatmap_3d_finetune_egopw_mo2cap2'))
    ])
...
model = dict(
    ...
    keypoint_head=dict(
        ...
        loss_keypoint=dict(type='PAMPJPELoss', use_target_weight=True)
        ),
    train_cfg=dict(),
    test_cfg=dict()
)
...
egopw_finetune_dataset_cfg = dict(
...
mo2cap2_test_dataset_cfg = dict(
        type='Mo2Cap2TestDataset',
        data_cfg=data_cfg,
        pipeline=train_pipeline,
        test_mode=False,
    )
...
data = dict(
    ...
    train=dict(
        type='ConcatDataset',
        datasets=[mo2cap2_test_dataset_cfg],
    ),
    val=dict(
        type='Mo2Cap2TestDataset',
        data_cfg=data_cfg,
