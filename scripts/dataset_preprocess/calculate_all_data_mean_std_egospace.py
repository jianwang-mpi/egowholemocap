...
def calculate_egobody_mean_std():
    ...
    egobody_pipeline = [
        ...
        Collect(keys=['smplx_output', 'smplx_input', 'ego_camera_rot', 'ego_camera_transl', 'ego_camera_transform',
                ...
                meta_keys=['ego_camera_transform', 'ego_smplx_joints',])
    ]

    egobody_dataset_cfg = dict(
        type='EgoBodyDataset',
        data_path=data_path,
        seq_len=seq_len,
    ...
    studio_motion_pipeline = [
        SplitGlobalSMPLXJoints(smplx_joint_name='ego_smplx_joints'),
        PreProcessHandMotion(normalize=normalize),
        PreProcessMo2Cap2BodyMotion(normalize=normalize),
        Collect(keys=['ego_smplx_joints',
                      'left_hand_keypoints_3d', 'right_hand_keypoints_3d',
                      'left_hand_features', 'right_hand_features', 'mo2cap2_body_features'],
                meta_keys=['ego_smplx_joints', ])
    ]
    ...
    studio_motion_dataset_config = dict(
        ...
        seq_len=seq_len,
        skip_frames=seq_len,
        pipeline=studio_motion_pipeline,
        split_sequence=split_sequence,
        test_mode=False,
        local=False,
        data_cfg={
    ...
    dataset_cfg = dict(
        type='ConcatDataset',
        datasets=[egobody_dataset_cfg, studio_motion_dataset_config],
    )
...
if __name__ == '__main__':
    if os.name == 'nt':
        ...
        save_mean_std_path = r'Z:\EgoMocap\work\EgocentricFullBody\dataset_files\diffusion_fullbody\ego_mean_std.pkl'
        save_data_path = r'Z:\EgoMocap\work\EgocentricFullBody\dataset_files\diffusion_fullbody\ego_feature_seqs_196.pkl'
    else:
        data_path = '/CT/datasets04/static00/EgoBody'
        smplx_model_dir = '/CT/EgoMocap/work/EgocentricFullBody/human_models/smplx_new'
        save_mean_std_path = '/CT/EgoMocap/work/EgocentricFullBody/dataset_files/diffusion_fullbody/ego_mean_std.pkl'
        save_data_path = '/CT/EgoMocap/work/EgocentricFullBody/dataset_files/diffusion_fullbody/ego_feature_seqs_196.pkl'
