...
def main():
    ...
    diffusion_mpjpe_result = [x for x in diffusion_mpjpe_result if x['mo2cap2_output_mpjpe'] < 0.06]
    diffusion_mpjpe_result_sorted = natsorted(diffusion_mpjpe_result,
                                              key=lambda x: x['mo2cap2_input_mpjpe'] - x['mo2cap2_output_mpjpe'],
                                              reverse=True)


    selected = diffusion_mpjpe_result_sorted[3]
    print(selected)
    ...
    for ext_id in selected['ext_id']:
        ...
        if not camera_setup:
            ...
            save_view_point([mo2cap2_gt_skeleton, coord], view_point_save_path)
            camera_setup = True
