...
def render_joints(sceneego_joints, our_single_joints, our_diff_joints, save_dir, image_file_name,
                  human_name, seq_name,
                  save=True, render=True, save_viewpoint=True, our_single_left_hand=None, our_single_right_hand=None,
...
def main():
    if os.name == 'nt':
        ...
        save_dir = r'Z:\EgoMocap\work\EgocentricFullBody\vis_results\egopw_diffusion_results_new'
    else:
        ...
        save_dir = '/CT/EgoMocap/work/EgocentricFullBody/vis_results/egopw_diffusion_results_new'
    ...
    for i in range(1250, 1550, 1):
        ...
        pkl_name = our_diff_result[i]['pkl_name']
        print('pkl_name_{}_id: {}_{}_{}_{}'.format(pkl_name, i, data_dict_human_name, seq_name, image_name))
        ...
        sceneego_data_path = os.path.join(data_dir, dir_name, human_name, seq_name, 'out', 'estimated_depth_our_method.pkl')
        sceneego_results = read_sceneego_result(sceneego_data_path)
        image_id = int(image_name.split('.')[0].split('_')[-1])
        sceneego_pose = sceneego_results[image_id]

        # visualize_joints(egopw_pose, sceneego_pose, our_single_pose, our_diff_pose)
        render_joints(sceneego_pose, our_single_pose, our_diff_pose, save_dir=save_dir,
                      human_name=dir_name + human_name, seq_name=seq_name, image_file_name=image_name,
                      save=True,
                      render=True,
