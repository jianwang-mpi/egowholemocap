...
def main():
    ...
    with open(pkl_path, 'rb') as f:
        data = pickle.load(f)
    pred_joints_3d_list = []
    fitted_mo2cap2_joint_list = []
    pred_left_hand_joint_3d_list = []
    pred_right_hand_joint_3d_list = []
    gt_joints_3d_list = []
    for pred in data:
        pred_joints_3d_item = pred['body_pose_results']['keypoints_pred']
        fitted_smplx_joints = pred['smplx_ik']['fitted_smplx_joints'].cpu().numpy()
        fitted_mo2cap2_joints = np.zeros((fitted_smplx_joints.shape[0], 15, 3))
        fitted_mo2cap2_joints[:, mo2cap2_smplx_dst_idxs] = fitted_smplx_joints[:, smplx_model_idxs]
        ...
        fitted_mo2cap2_joint_list.extend(fitted_mo2cap2_joints)
    ...
    fitted_mo2cap2_joint_list = np.array(fitted_mo2cap2_joint_list)
    ...
    mpjpe = keypoint_mpjpe(fitted_mo2cap2_joint_list, gt_joints_3d_list, mask)

    print(mpjpe)
    pa_mpjpe = keypoint_mpjpe(fitted_mo2cap2_joint_list, gt_joints_3d_list, mask, alignment='procrustes')
    print(pa_mpjpe)
    ...
    for i in range(0, len(pred_joints_3d_list)):
