...
result_path = r'Z:/EgoMocap/work/EgocentricFullBody/work_dirs/diffusion_left_hand_train_cond\outputs_9.pkl'
normalize = True
vis_skip_frame = 1
save = False
vis = False
save_viewpoint = False
save_dir = r'Z:/EgoMocap/work/EgocentricFullBody/vis_results/diffusion_hands/'
...
input_motion_sequence_list = []
diffusion_process_list = []
for result_block in results:
    result_motion_sequence_list.extend(result_block['sample'].cpu().numpy())
    input_motion_sequences = result_block['mo2cap2_body_features'].cpu().numpy()
    input_motion_sequence_list.extend(input_motion_sequences)
...
left_hand_motion_sequence_list = np.asarray(result_motion_sequence_list)
if left_hand_motion_sequence_list.shape[-1] == 20 * 3:
    left_hand_motion_sequence_list = left_hand_motion_sequence_list.reshape(-1, 196, 20, 3)
    left_hand_motion_sequence_list = np.concatenate(
        [left_hand_motion_sequence_list, np.zeros((left_hand_motion_sequence_list.shape[0], 196, 1, 3))], axis=2)
    left_hand_motion_sequence_list = left_hand_motion_sequence_list.reshape(-1, 196, 21 * 3)
input_motion_sequence_list = np.asarray(input_motion_sequence_list)
...
if normalize:
    ...
    input_motion_sequence_list = input_motion_sequence_list * mo2cap2_body_std + mo2cap2_body_mean

left_hand_motion_list = left_hand_motion_sequence_list.reshape(-1, 196, 21, 3)
mo2cap2_motion_list = input_motion_sequence_list.reshape(-1, 196, 15, 3)
for motion_i, (left_hand_motion, mo2cap2_motion) in enumerate(zip(left_hand_motion_list, mo2cap2_motion_list)):
    if motion_i != motion_id:
        continue

    left_hand_motion[:, 0] *= 0

    assert len(left_hand_motion) == len(mo2cap2_motion)

    if render:
        render_image_save_dir = os.path.join(save_dir, 'render_imgs')
    ...
    for i in range(0, len(left_hand_motion), vis_skip_frame):
        ...
        if only_hand:
            ...
            overall_mesh = mo2cap2_mesh + left_hand_mesh

        if vis:
            open3d.visualization.draw_geometries([overall_mesh])
        if save:
            mesh_save_path = os.path.join(save_dir, 'hand_%04d.ply' % i)
            open3d.io.write_triangle_mesh(mesh_save_path, overall_mesh)
        if save_viewpoint:
            save_view_point([overall_mesh], view_point_save_path)
        if render:
    ...
    if render:
        subprocess.run(shlex.split(
            ...
            f'-vcodec libx264 -pix_fmt yuv420p -crf 25 -y '
