...
if os.name == "nt":
    network_pred_seq_path = r'Z:\EgoMocap\work\EgocentricFullBody\work_dirs\egofullbody_test_egopw\outputs.pkl'
    ...
    diffusion_result_save_dir = r"Z:/EgoMocap/work/EgocentricFullBody/vis_results/egopw_diffusion_results_new"
else:
    network_pred_seq_path = '/CT/EgoMocap/work/EgocentricFullBody/work_dirs/egofullbody_test_egopw/outputs.pkl'
    ...
    diffusion_result_save_dir = "/CT/EgoMocap/work/EgocentricFullBody/vis_results/egopw_diffusion_results_new"
...
def get_fullbody_motion_test_dataset():
    ...
    pipeline = [
        ...
        Collect(keys=['mo2cap2_body_features', 'left_hand_features', 'right_hand_features',
                ...
                meta_keys=['name', 'image_name', 'image_path', 'seq_name', 'human_name'])
    ]

    dataset_cfg = dict(
        type='FullBodyEgoPwMotionTestDataset',
        egopw_pred_data_pkl_path=network_pred_seq_path,
        seq_len=seq_len,
        skip_frames=seq_len,
        pipeline=pipeline,
        split_sequence=True,
...
def run_diffusion(data_id):
    ...
    with open(os.path.join(diffusion_result_save_dir, f"new_diffusion_results_{data_id}.pkl"), 'wb') as f:
...
def run_whole_sequence_and_evaluate():
    ...
    for data_id_input in range(95, 100):
        run_diffusion(data_id_input)
        data_pkl_path = os.path.join(diffusion_result_save_dir, f"new_diffusion_results_{data_id_input}.pkl")
