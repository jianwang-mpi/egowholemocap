...
from mmpose.models.diffusion_mdm.data_loaders.humanml.common.quaternion import qinv_np
...
if os.name == "nt":
    ...
    diffusion_result_save_global_dir = "/CT/EgoMocap/work/EgocentricFullBody/vis_results/egofullbody_diffusion_results_global"
...
def run_diffusion(data_id):
    ...
    data_preprocess_rot_quat_inv_list = np.repeat(data_preprocess_rot_quat_inv_list[:, None, :],
                                                  15+21+21, axis=1)
    data_preprocess_trans_list = data_preprocess_trans_list.cuda().float()
    data_preprocess_rot_quat_inv_list = torch.from_numpy(data_preprocess_rot_quat_inv_list).cuda().float()
    data_i['root_trans'] = data_preprocess_trans_list
    data_i['root_rot'] = data_preprocess_rot_quat_inv_list

    from mmpose.models.diffusion_hands.refine_mo2cap2_hands_foot_sliding import RefineEdgeDiffusionHands
    full_body_pose_diffusion_refiner = RefineEdgeDiffusionHands(seq_len=196,
                                                                mean_std_path=mean_std_path,
                                                                visualize=False,
...
def get_pred_gt_motion(data_pkl_path):
    ...
    data_preprocess_rot_quat_list = data_preprocess_rot_quat_list.detach().cpu().numpy()
    data_preprocess_rot_quat_inv_list = qinv_np(data_preprocess_rot_quat_list)
    ...
    combined_motion = combined_motion - data_preprocess_trans_list.detach().cpu().numpy()
