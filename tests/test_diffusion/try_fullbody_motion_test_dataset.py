...
def try_renderpeople_dataset(seq_id):
    seq_len = 196
    normalize = False
    pipeline = [
        AlignGlobalSMPLXJoints(align_every_joint=True, use_default_floor_height=True),
    ...
    dataset_cfg = dict(
        ...
        data_pkl_path=r'Z:\EgoMocap\work\EgocentricFullBody\work_dirs\egofullbody_test_b_256\results.pkl',
        seq_len=seq_len,
        skip_frames=seq_len,
        pipeline=pipeline,
        split_sequence=True,
    ...
    for frame_id in range(0, 1000, 100):
        ...
        # combine hand and body joints
        body_left_hand_feature = left_hand_feature + mo2cap2_feature[6:7]
        body_right_hand_feature = right_hand_feature + mo2cap2_feature[3:4]
        body_left_hand_mesh = draw_skeleton_with_chain(body_left_hand_feature, mano_skeleton, keypoint_radius=0.01,
                                                          line_radius=0.0025)
        body_right_hand_mesh = draw_skeleton_with_chain(body_right_hand_feature, mano_skeleton, keypoint_radius=0.01,
                                                           line_radius=0.0025)
        open3d.visualization.draw_geometries([aligned_mo2cap2_joints_mesh,body_left_hand_mesh, body_right_hand_mesh,
                                              coord])



if __name__ == '__main__':
    try_renderpeople_dataset(2)
