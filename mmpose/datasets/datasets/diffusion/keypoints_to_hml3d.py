...
# convert keypoint pose representation to HumanML3D representation
...
from mmpose.models.diffusion_mdm.data_loaders.humanml.utils.paramUtil import *
from mmpose.models.diffusion_mdm.data_loaders.humanml.common.quaternion import *
from mmpose.models.diffusion_mdm.data_loaders.humanml.common.skeleton import Skeleton
...
class KeypointsToHumanML3D:
    def __init__(self):
        example_id = "000021"

        if os.name == 'nt':
            example_data = r'Z:\EgoMocap\work\motion-diffusion-model\dataset\HumanML3D\new_joints\012314.npy'
        else:
            example_data = '/CT/EgoMocap/work/motion-diffusion-model/dataset/HumanML3D/new_joints/012314.npy'

        example_data = np.load(example_data)
    ...
    def process_file(self, positions, feet_thre):
        ...
        print(root_pos_init[sdr_r])
        print(root_pos_init[sdr_l])
        print(across2)
        ...
        # skeleton_mesh = draw_skeleton_with_chain(root_pos_init, self.kinematic_chain)
        # coord = open3d.geometry.TriangleMesh.create_coordinate_frame()
        # arrow = get_arrow(vec=across1)
        # open3d.visualization.draw_geometries([skeleton_mesh, coord, arrow])


        # forward (3,), rotate around y-axis
        forward_init = np.cross(np.array([[0, 1, 0]]), across, axis=-1)
        # debug here
        # forward_init = np.cross(across, np.array([[0, 1, 0]]), axis=-1)
        # forward (3,)
        ...
        # skeleton_mesh = draw_skeleton_with_chain(positions[0], self.kinematic_chain)
        # coord = open3d.geometry.TriangleMesh.create_coordinate_frame()
