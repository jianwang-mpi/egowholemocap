...
class GridGenerator:
    ...
    def createSamplingPattern(self):
        ...
        lat_range = ((h_range / self.height) - 0.5) * np.pi / 2
        ...
        lon = np.array([
            np.arctan(kerX * sin_nu / (rho * np.cos(_lat) * cos_nu - kerY * np.sin(_lat) * sin_nu)) for _lat in
            lat_range
        ])  # (H, Kh, Kw)
...
if __name__ == '__main__':
    generator = GridGenerator(height=10, width=10, kernel_size=(5, 5))
    pattern = generator.createSamplingPattern()
    print(pattern)