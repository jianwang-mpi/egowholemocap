...
from mmpose.models.expose.data.targets import ImageList, ImageListPacked
from mmpose.models.expose.data.targets.keypoints import KEYPOINT_NAMES, get_part_idxs
from mmpose.models.expose.data.utils import flip_pose, bbox_iou, center_size_to_bbox
...
class SMPLXHead(nn.Module):
    ...
    def __init__(
        ...
        transl_idxs = list(range(start, start + 3))
        self.register_buffer(
            'transl_idxs', torch.tensor(transl_idxs, dtype=torch.long))
        start += 3
        mean_lst.append(torch.as_tensor([0, 0.22, 0.81], dtype=dtype))
    ...
    def flat_body_params_to_dict(self, param_tensor):
        ...
        transl = torch.index_select(param_tensor, 1, self.transl_idxs)
    ...
    def forward(self,
        ...
        # print(body_features.requires_grad)

        body_parameters, body_deltas = self.regressor(body_features)

        # print(body_parameters[0].requires_grad)
        ...
        if self.training and self.predict_head and self.predict_hands:
