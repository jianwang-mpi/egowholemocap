...
from mmpose.models.ego_hand_pose_estimation.nets.layer import make_conv_layers, make_linear_layers
from mmpose.models.ego_hand_pose_estimation.utils.human_models import HumanModels
from mmpose.models.ego_hand_pose_estimation.utils.transforms import sample_joint_features, soft_argmax_3d
...
class PositionNet(nn.Module):
    def __init__(self):
        ...
        if cfg.parts == 'body':
            self.joint_num = self.human_models.smpl.pos_joint_num
        elif cfg.parts == 'hand':
            self.joint_num = self.human_models.mano.joint_num
...
class RotationNet(nn.Module):
    def __init__(self):
        ...
        if cfg.parts == 'body':
            ...
            self.pose_out = make_linear_layers([self.joint_num*(512+3), (self.human_models.smpl.orig_joint_num-3)*6], relu_final=False) # without root and two hands
            self.shape_out = make_linear_layers([2048,self.human_models.smpl.shape_param_dim], relu_final=False)
            self.cam_out = make_linear_layers([2048,3], relu_final=False)
        elif cfg.parts == 'hand':
            self.conv = make_conv_layers([2048,512], kernel=1, stride=1, padding=0)
            self.root_pose_out = make_linear_layers([self.joint_num*(512+3), 6], relu_final=False)
            self.pose_out = make_linear_layers([self.joint_num*(512+3), (self.human_models.mano.orig_joint_num-1)*6], relu_final=False) # without root joint
            self.shape_out = make_linear_layers([2048,self.human_models.mano.shape_param_dim], relu_final=False)
            self.cam_out = make_linear_layers([2048,3], relu_final=False)
...
class FaceRegressor(nn.Module):
    def __init__(self):
        ...
        self.shape_out = make_linear_layers([2048, self.human_models.flame.shape_param_dim], relu_final=False) # shape parameter
        self.expr_out = make_linear_layers([2048, self.human_models.flame.expr_code_dim], relu_final=False) # expression parameter
