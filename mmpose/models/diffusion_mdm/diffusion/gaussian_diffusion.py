...
from mmpose.models.diffusion_mdm.diffusion.nn import mean_flat, sum_flat
from mmpose.models.diffusion_mdm.diffusion.losses import normal_kl, discretized_gaussian_log_likelihood
from mmpose.models.diffusion_mdm.data_loaders.humanml.scripts import motion_process
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        #     generated_motion = model_output * ~inpainting_mask + inpainted_motion * inpainting_mask
        #     model_output = generated_motion


        if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
            inpainting_mask = deepcopy(model_kwargs['y']['inpainting_mask'])
            inpainted_motion = deepcopy(model_kwargs['y']['inpainted_motion'])
            assert self.model_mean_type == ModelMeanType.START_X, 'This feature supports only X_start pred for mow!'
            assert model_output.shape == inpainting_mask.shape == inpainted_motion.shape
            # model_output = (model_output * ~inpainting_mask) + (inpainted_motion * inpainting_mask)
            ...
            inpainting_mask[:, :, :, :50] *= 1
            inpainting_mask[:, :, :, 120:] *= 2
            inpainting_mask_smooth = 1 / (1 + torch.exp(-1 * (time_step - 1000 * (1 - inpainting_mask))))

            generated_motion = model_output * (1 - inpainting_mask_smooth) + inpainted_motion * inpainting_mask_smooth
            model_output = generated_motion
