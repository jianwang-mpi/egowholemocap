...
class UndistortPatchOld(nn.Module):
    def __init__(self,
    ...
    def _create_sampling_patches(self, patch_num_horizontal=16, patch_num_vertical=16,
        ...
        for i, patch_center in enumerate(patch_centers_3d_list):
            _, patch_center_lat, patch_center_lon = self.cart2polar(patch_center)
            patch_coordinates = self._generate_patch_coordinates(patch_center_lat, patch_center_lon,
    ...
    def cart2polar(self, coordinate):
        x, y, z = coordinate[0], coordinate[1], coordinate[2]
        r = math.sqrt(x * x + y * y + z * z)
        lat = math.asin(z / r) - np.pi / 2
        lon = math.atan2(y, x)
        return r, lat, lon
