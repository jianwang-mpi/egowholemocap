...
#     """
#
#     def __init__(self, patch_num_lat, patch_num_lon, patch_size=32, in_chans=3, embed_dim=768):
#         super().__init__()
#         patch_size = to_2tuple(patch_size)
#         num_patches = (patch_num_lat - 1) * patch_num_lon
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_num_lat, patch_num_lon, patch_size=32, in_chans=3, embed_dim=768, ratio=1):
        super().__init__()
        patch_size = to_2tuple(patch_size)
        num_patches = patch_num_lat * patch_num_lon
        self.patch_size = patch_size
        self.num_patches = num_patches
        self.embed_dim = embed_dim
        in_dims = in_chans * patch_size[0] * patch_size[1]
...
class FisheyeViT(BaseBackbone):
    def __init__(self,
        ...
        super(FisheyeViT, self).__init__()
        ...
        assert embed_type in ['linear', 'cnn']
        if embed_type == 'linear':
            self.patch_embed = PatchEmbed(self.fisheye2sphere_config['patch_num_lat'],
                                          self.fisheye2sphere_config['patch_num_lon'],
                                          patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
                                          ratio=ratio)
        else:
            self.patch_embed = PatchEmbedWithCNN(self.fisheye2sphere_config['patch_num_lat'],
                                                 self.fisheye2sphere_config['patch_num_lon'],
                                                 patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
    ...
    def forward_features(self, x):
        ...
        B, patch_num, C, H, W = x.shape
        Hp = self.fisheye2sphere_config['patch_num_lat']
        Wp = self.fisheye2sphere_config['patch_num_lon']
...
class ResizeTransformerPatches(nn.Module):
    """
    Resize the patch number from the transformer
    """

    def __init__(self, input_feature, output_feature, with_norm=False, activation='none', reshape=(16, 16)):
        super().__init__()
        self.input_feature = input_feature
        self.output_feature = output_feature
        self.linear = nn.Linear(input_feature, output_feature, )
        self.norm = nn.BatchNorm1d(self.output_feature) if with_norm else nn.Identity()
        assert activation in ['relu', 'sigmod', 'none']
        if activation == 'relu':
        ...
        if self.reshape is not None:
            assert self.reshape[0] * self.reshape[1] == output_feature
    ...
    def forward(self, x):
        B, patch_num, Hp, Wp = x.shape
        x = x.reshape(B, patch_num, Hp * Wp)
        x = x.permute(0, 2, 1)
        x = self.linear(x)
        x = self.norm(x)
        x = self.activation(x)
        if self.reshape is not None:
            x = x.reshape(-1, feature_num, self.reshape[0], self.reshape[1])
        return x
