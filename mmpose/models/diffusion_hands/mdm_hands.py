...
class DiffusionHands(BasePose):
    def __init__(self, right_hand=True, load_model_path=None, representation_dim=21 * 3,
                 cond_feature_dim=15 * 3,
                 guidance_weight=0,
                 cond_drop_prob=0.25,
                 seq_len=196,
                 return_diffusion=False):
        super(DiffusionHands, self).__init__()
    ...
    def forward_train(self, left_hand_features, right_hand_features, mo2cap2_body_features, img_metas, **kwargs):
        if self.right_hand:
            loss_dict = self.diffusion_model(right_hand_features, cond=mo2cap2_body_features)
        else:
            loss_dict = self.diffusion_model(left_hand_features, cond=mo2cap2_body_features)
        return loss_dict

    def forward_test(self, left_hand_features, right_hand_features, mo2cap2_body_features, img_metas, **kwargs):
        batch_size = mo2cap2_body_features.shape[0]
        if self.right_hand:
            sample_shape = right_hand_features.shape
        else:
            sample_shape = left_hand_features.shape
