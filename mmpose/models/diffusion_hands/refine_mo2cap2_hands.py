...
class RefinerJointPosition:
    def __init__(self, joint_3d_seq, joint_3d_seq_confidence, k=0.1, total_timesteps=1000):
...
class RefineEdgeDiffusionHands(BasePose):
    ...
    def forward(self, mo2cap2_body_features, left_hand_features, right_hand_features,
        ...
        joint_3d_seq_confidence[:, :, 3 * 3: 4 * 3] = 0.998
        joint_3d_seq_confidence[:, :, 6 * 3: 7 * 3] = 0.998
        joint_3d_seq_confidence[:, :, 7 * 3: 15 * 3] = 0.996
        joint_3d_seq_confidence[:, :, 15 * 3:] = 0.997
        # joint_3d_seq_confidence[:, 30: 50, 15 * 3: (15 + 21) * 3] = 0.998
        # joint_3d_seq_confidence = torch.ones_like(features_all).to(features_all.device) * 1
        # joint_3d_seq_confidence[:, :, 7 * 3: 15 * 3] = 0.998
        # joint_3d_seq_confidence[:, :, 15 * 3:] = 0.998
        joint_position_refiner = RefinerJointPosition(k=0.1, joint_3d_seq=features_all,
