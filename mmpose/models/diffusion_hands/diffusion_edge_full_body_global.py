...
class DiffusionEDGEFullBodyGlobal(BasePose):
    def __init__(self, load_model_path=None, representation_dim=(21 + 21 + 15) * 3,
        ...
        super(DiffusionEDGEFullBodyGlobal, self).__init__()
    ...
    def forward_train(self, root_features, left_hand_features, right_hand_features, mo2cap2_body_features,
                      img_metas, **kwargs):

        # combine all features
        features_all = torch.cat([root_features, mo2cap2_body_features,
                                  left_hand_features, right_hand_features], dim=-1)
        loss_dict = self.diffusion_model(features_all, cond=mo2cap2_body_features)
        return loss_dict

    def forward_test(self, root_features, left_hand_features, right_hand_features, mo2cap2_body_features, img_metas, **kwargs):
        # combine all features
        features_all = torch.cat([root_features, mo2cap2_body_features,
                                  left_hand_features, right_hand_features], dim=-1)
    ...
    def forward(self, root_features, left_hand_features, right_hand_features, mo2cap2_body_features,
                img_metas=None, return_loss=True, **kwargs):
        if return_loss:
            return self.forward_train(root_features, left_hand_features, right_hand_features, mo2cap2_body_features,
                                      img_metas, **kwargs)
        else:
            return self.forward_test(root_features, left_hand_features, right_hand_features, mo2cap2_body_features,
                                     img_metas, **kwargs)
