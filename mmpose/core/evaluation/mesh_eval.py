...
def compute_similarity_transform_torch(source_points_torch, target_points_torch):
    """Computes a similarity transform (sR, t) that takes a set of 3D points
    source_points (N x 3) closest to a set of 3D points target_points, where R
    is an 3x3 rotation matrix, t 3x1 translation, s scale. And return the
    transformed 3D points source_points_hat (N x 3). i.e. solves the orthogonal
    ...
    Args:
        source_points (torch.Tensor): Source point set with shape [N, 3].
        target_points (torch.Tensor): Target point set with shape [N, 3].

    Returns:
        torch.Tensor: Transformed source point set with shape [N, 3].
    """
    source_points_torch = source_points_torch.float()
    target_points_torch = target_points_torch.float()
    assert target_points_torch.shape[0] == source_points_torch.shape[0]
    assert target_points_torch.shape[1] == 3 and source_points_torch.shape[1] == 3

    source_points = source_points_torch.detach().cpu().numpy()
    target_points = target_points_torch.detach().cpu().numpy()
    ...
    # 7. Transform the source points:
    R_torch = torch.from_numpy(R).float().to(source_points_torch.device)
    t_torch = torch.from_numpy(t).float().to(source_points_torch.device)
    scale_float = float(scale)
    source_points_torch_hat = scale_float * R_torch @ source_points_torch.T + t_torch
    # print('source: ')
    # print(source_points_torch)
    # print('target: ')
    # print(target_points_torch)
    # print('aligned: ')
    # print(source_points_torch_hat)

    source_points_torch_hat = source_points_torch_hat.T

    return source_points_torch_hat