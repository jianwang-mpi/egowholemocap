...
def keypoint_mpjpe(pred, gt, mask, alignment='none'):
    ...
    elif alignment == 'bone_length':
        # calculate ba-mpjpe, align the bone length
        print('calculate ba-mpjpe, align the bone length')
        skeleton_model = Skeleton(None)
        for i in range(len(pred)):
            pred[i] = skeleton_model.skeleton_resize_single(
                pred[i],
                bone_length_file='/CT/EgoMocap/work/EgocentricFullBody/mmpose/utils/fisheye_camera/mean3D.mat')
        for i in range(len(gt)):
            gt[i] = skeleton_model.skeleton_resize_single(
                gt[i],
                bone_length_file='/CT/EgoMocap/work/EgocentricFullBody/mmpose/utils/fisheye_camera/mean3D.mat')
        pred = np.stack([
            compute_similarity_transform(pred_i, gt_i)
...
def keypoint_3d_pck(pred, gt, mask, alignment='none', threshold=0.15):
    ...
    if alignment == 'none':
        ...
        for i in range(len(pred)):
            pred[i] = skeleton_model.skeleton_resize_single(
                pred[i],
                bone_length_file='utils/fisheye_camera/mean3D.mat')
        for i in range(len(gt)):
            gt[i] = skeleton_model.skeleton_resize_single(
                gt[i],
                bone_length_file='utils/fisheye_camera/mean3D.mat')
