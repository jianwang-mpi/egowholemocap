...
def _get_softargmax_preds(heatmaps):
    # heatmap shape: N x K x H x W
    height, width = heatmaps.shape[2:]

    # normalize the heatmaps
    heatmaps = heatmaps / np.sum(heatmaps, axis=(2, 3), keepdims=True)

    accu_x = np.sum(heatmaps, axis=2)
    accu_y = np.sum(heatmaps, axis=3)

    accu_x = accu_x * np.arange(width)[None, None, :]
    accu_y = accu_y * np.arange(height)[None, None, :]

    accu_x = np.sum(accu_x, axis=2, keepdims=True)
    accu_y = np.sum(accu_y, axis=2, keepdims=True)

    coord_out = np.concatenate((accu_x, accu_y), axis=2)
